...
@ComponentProfile(summary = "Use the Elastic SDK to interact with an ElasticSearch instance", tag = "producer,elastic,elasticsearch",
    since = "4.6", recommended = {ElasticConnection.class})
@DisplayOrder(order =
{
...
public class ElasticSdkProducer extends RequestReplyProducerImp {
  ...
  /**
   * The action for this operation if not explicitly defined by the
   * {@link DocumentWrapper}
   * <p>
   * If not explicitly defined then defaults to {@link ConfiguredAction} with a
   * default of {@code INDEX}.
   * </p>
   */
  @Valid
  @Getter
    ...
    try {
      ...
      try (CloseableIterable<DocumentWrapper> docs = CloseableIterable.ensureCloseable(documentBuilder.build(msg))) {
        for (DocumentWrapper doc : docs) {
          ...
          DocumentAction action = ObjectUtils.defaultIfNull(doc.action(),
              DocumentAction.valueOf(actionExtractor().extract(msg, doc)));

          switch (action) {
          case INDEX: {
            IndexResponse response = client.index(requestBuilder.buildIndexRequest(endpoint, doc, getRefreshPolicy(), client, msg.getContentEncoding()));
            log.trace("INDEX:: document {} version {} in {}", response.id(), response.version(), endpoint);
            checkResponse(response);
            break;
          }
          case UPDATE: {
            UpdateResponse response = client.update(requestBuilder.buildUpdateRequest(endpoint, doc, getRefreshPolicy(), client, msg.getContentEncoding()), Void.class);
          ...
          case UPSERT: {
            UpdateResponse response = client.update(requestBuilder.buildUpsertRequest(endpoint, doc, getRefreshPolicy(), client, msg.getContentEncoding()), Void.class);
